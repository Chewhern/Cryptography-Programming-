...
Public Class AEAD_AES
    Private Sub GoBTN_Click(sender As Object, e As EventArgs) Handles GoBTN.Click
        ...
        If DataString <> "" Then
            ...
            SecretBoxNonce = SecretAead.GenerateNonce
            If AdditionalDataString <> "" Then
                ...
                If AdditionalDataByte.Length <= 16 Then
                    EncryptedDataByte = SecretAead.Encrypt(DataByte, SecretBoxNonce, SecretBoxKey, AdditionalDataByte)
                    EncryptedDataBigInt = New BigInteger(EncryptedDataByte)
                    DecryptedDataByte = SecretAead.Decrypt(EncryptedDataByte, SecretBoxNonce, SecretBoxKey, AdditionalDataByte)
                ...
                Else
                    ...
                    EncryptedDataByte = SecretAead.Encrypt(DataByte, SecretBoxNonce, SecretBoxKey)
                    EncryptedDataBigInt = New BigInteger(EncryptedDataByte)
                    DecryptedDataByte = SecretAead.Decrypt(EncryptedDataByte, SecretBoxNonce, SecretBoxKey)
                    DecryptedDataBigInt = New BigInteger(DecryptedDataByte)
                    DecryptedData = Encoding.UTF8.GetString(DecryptedDataByte)
                End If
            Else
